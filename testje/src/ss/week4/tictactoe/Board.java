...
public class Board {
    ...
    public Mark getField(int row, int col) {
    	if (this.isField(row, col)) {
    		return fields[this.index(row, col)];
    	}
    	return null;
    }
    ...
    public boolean isEmptyField(int row, int col) {
    	if (this.isField(row, col)) {
    		return (this.getField(this.index(row, col)) == Mark.EMPTY);
    	}
    	return false;
    }
    ...
    public boolean hasDiagonal(Mark m) {
    	...
    	if (streak == DIM) {
    		...
    		streak = 0;
	    	for (int i = 0; i < DIM; i++) {
	    		if (getField(i, DIM - 1 - i).equals(m)) {
	    			streak++;
	    		}
	    	}
	    	if (streak == DIM) {
    ...
    public void setField(int row, int col, Mark m) {
    	if (this.isField(this.index(row, col))) {
